<%@ Page Language="C#" AutoEventWireup="true" CodeFile="frontList.aspx.cs" Inherits="Student_frontList" %>
			...
			<div class="col-md-3 bottom15" <%#(((Container.ItemIndex+0)%4==0)?"style='clear:left;'":"") %>>
			  <a href="frontshow.aspx?user_name=<%#Eval("user_name")%>"><img class="img-responsive" src="../<%#Eval("userPhoto")%>" /></a>
			     <div class="showFields">
			        ...
			        <a class="btn btn-primary top5" onclick="studentEdit('<%#Eval("user_name")%>');" style="display:none;">修改</a>
			        <a class="btn btn-primary top5" onclick="studentDelete('<%#Eval("user_name")%>');" style="display:none;">删除</a>
			     </div>
			</div>
			</ItemTemplate>
			</asp:Repeater>
...
function studentEdit(user_name) {
	$.ajax({
		url :  basePath + "Student/StudentController.aspx?action=getStudent&user_name=" + user_name,
		type : "get",
		dataType: "json",
...
function studentDelete(user_name) {
	if(confirm("确认删除这个记录")) {
		$.ajax({
			type : "POST",
			url : basePath + "Student/StudentController.aspx?action=delete",
			data : {
				user_name : user_name,
			},
			dataType: "json",
			success : function (obj) {
				if (obj.success) {
					alert("删除成功");
                    $("#btnSearch").click();
					//location.href= basePath + "Student/frontList.aspx";
				}
				else 
					alert(obj.message);
			},
...
function ajaxStudentModify() {
	$.ajax({
		url :  basePath + "Student/StudentController.aspx?action=update",
		type : "post",
		dataType: "json",
